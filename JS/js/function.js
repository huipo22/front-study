...
/*	function getChilds(father,type){
	...
	function getDown(obj){
		...
		while(down.nodeType==3||down.nodeType==8){//是文本节点和注释节点 ----一直往下找
			down=down.nextSibling;//接着往下面找---down.nextSibling
			if(down==null){
				return false;
			}
		} 
	...
	//10.获取上一个兄弟节点
	function getUp(obj){
		var up=obj.previousSibling;//获取上一个兄弟元素
		if(up==null){//如果为空  返回false
			return false
		}
		while(up.nodeType==3||up.nodeType==8){//是文本节点和注释节点 ----一直往上找
